...
function setupTouchEvents() {
    ...
    if (window.isMobile) {
        ...
        // הצגת השלב הנוכחי מחדש
        setTimeout(() => {
            const currentStepNumber = parseInt(localStorage.getItem('currentStep') || '1');
...
function sendToCustomerWhatsApp(userDetails) {
    try {
        ...
        message += `סוג משלוח: ${userDetails.deliveryMethod === 'express' ? 'משלוח מהיר' : 'משלוח רגיל'}\n`;
        message += `מחיר סופי: ₪${(finalPrice + (userDetails.deliveryMethod === 'express' ? 100 : 50)).toLocaleString()}\n\n`;
...
function sendToCustomerEmail(userDetails) {
    try {
        ...
        body += `סוג משלוח: ${userDetails.deliveryMethod === 'express' ? 'משלוח מהיר' : 'משלוח רגיל'}\n`;
        body += `מחיר סופי: ₪${(finalPrice + (userDetails.deliveryMethod === 'express' ? 100 : 50)).toLocaleString()}\n\n`;
...
function startCountdown() {
    // הגדרת תאריך סיום הקמפיין - 31 יום מהיום
    const endDate = new Date();
    endDate.setDate(endDate.getDate() + 31);
    endDate.setHours(23, 59, 59, 0);
    
    // שמירת תאריך הסיום ב-localStorage כדי שיהיה עקבי בין טעינות דף
    if (!localStorage.getItem('campaignEndDate')) {
        localStorage.setItem('campaignEndDate', endDate.getTime().toString());
    }
    
    // קבלת תאריך הסיום מה-localStorage אם קיים
    const savedEndDate = localStorage.getItem('campaignEndDate');
    const finalEndDate = savedEndDate ? new Date(parseInt(savedEndDate)) : endDate;
    
    // עדכון הטיימר בפעם הראשונה
    updateCountdown(finalEndDate);
